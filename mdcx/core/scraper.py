...
class Scraper:
    ...
    async def _run(self, file_mode: FileMode, movie_list: list[Path] | None) -> None:
        ...
        # 网站健康检查（非单文件模式时执行）
        if file_mode != FileMode.Single and manager.config.check_site_health:
            signal.show_log_text("🏥 正在检查网站可达性...")
            from .health import HealthChecker
            health_checker = HealthChecker(manager.config, manager.computed.async_client)
            health_map = await health_checker.check_configured_sites(timeout=5.0)
            health_checker.log_health_status(health_map)
            # 获取可达网站列表，用于后续刮削
            Flags.reachable_sites = health_checker.get_reachable_sites(health_map)
    ...
    async def _process_one_file(
        ...
        # 验证刮削结果完整性：title 必须存在
        if not res.title:
            LogBuffer.error().write("刮削数据不完整：未获取到标题，跳过文件处理")
            return None, None
