...
async def download_file_with_filepath(url: str, file_path: Path, folder_new_path: Path) -> bool:
    ...
    try:
        if await manager.computed.async_client.download(url, file_path):
            return True
        # 对于 wp.com CDN，尝试去掉 resize 参数重试
        if "wp.com" in url and "resize=" in url:
            url_no_resize = re.sub(r"[?&]resize=\d+%2C\d+", "", url)
            url_no_resize = re.sub(r"^\?", "&", url_no_resize) if url_no_resize.startswith("?") else url_no_resize
            if await manager.computed.async_client.download(url_no_resize, file_path):
                return True
    except Exception:
        pass
